# Laboratorio 1: Consultas sobre tablas
...
| **Complejidad** | Fácil |
| **Nivel Bloom** | Aplicar |
| **Módulo** | 2 — Fundamentos de SQL con PostgreSQL |
| **Orden en secuencia** | 2 de 10 |
...
- Laboratorio 01-00-01 completado: entorno Docker con PostgreSQL 16 funcional y accesible
...
| Almacenamiento libre | Mínimo 2 GB para el dataset y logs |
...
| psql CLI | 16.x (incluido en imagen Docker) | Ejecutar scripts y consultas SQL |
| pgAdmin 4 | 7.x o superior | Interfaz gráfica alternativa para consultas |
| Visual Studio Code | 1.85 o superior | Editar scripts SQL antes de ejecutarlos |
...
> ⚠️ **Nota de Seguridad:** Este laboratorio usa las credenciales `usuario: postgres / contraseña: postgres` exclusivamente para entornos de desarrollo local. **Nunca uses estas credenciales en un entorno de producción.**
...
**Objetivo:** Crear la base de datos `ventas_db` y las seis tablas del dataset de ventas minoristas con sus relaciones de clave foránea.
...
**Objetivo:** Poblar las seis tablas con datos de prueba representativos de un negocio de ventas minoristas para poder realizar consultas analíticas significativas.
...
**Objetivo:** Practicar la escritura de consultas SELECT con filtros usando operadores de comparación y lógicos, aplicando las convenciones de estilo de la Lección 2.1.
...
**Objetivo:** Implementar paginación en resultados de consultas usando `LIMIT` y `OFFSET` para simular la recuperación de datos por páginas, como en una aplicación web.
...
**Objetivo:** Combinar información de múltiples tablas usando los cuatro tipos de JOIN para obtener vistas enriquecidas del dataset de ventas.
...
**Objetivo:** Identificar y manejar correctamente los valores NULL en el dataset, usando `IS NULL`, `IS NOT NULL` y la función `COALESCE` para reemplazar NULLs con valores por defecto.
...
**Objetivo:** Calcular métricas de negocio usando funciones de agregación (`COUNT`, `SUM`, `AVG`, `MIN`, `MAX`) con agrupaciones por categoría, región y período, filtrando grupos con `HAVING`.
...
**Objetivo:** Aplicar de forma autónoma las técnicas aprendidas en los pasos anteriores para responder preguntas de negocio sin guía paso a paso.
...
- **Laboratorio 03-00-01:** Profundizarás en consultas avanzadas usando subconsultas correlacionadas, CTEs (`WITH`) y las primeras funciones de ventana (`OVER`, `PARTITION BY`) sobre el mismo dataset de ventas.
- **Práctica recomendada:** Intenta escribir 3 consultas adicionales que respondan preguntas de negocio propias sobre el dataset (por ejemplo: ¿Cuál es el ticket promedio por ciudad? ¿Qué vendedor tiene la mayor tasa de cancelación?).
- **Reto integrador (evaluación formativa):** Ver sección de reto a continuación.
