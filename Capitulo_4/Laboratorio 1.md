# Laboratorio 1: Creación y Uso de Vistas y Vistas Materializadas
...
- Laboratorios del Módulo 3 completados (03-00-01, 03-00-02, 03-00-03, 03-00-04)
...
- Usuario `postgres` con contraseña `postgres` y acceso a la base de datos `sales_db`
...
> ⚠️ **Importante:** Si alguna tabla aparece con 0 registros o no existe, debes completar los laboratorios anteriores antes de continuar. Consulta el script de restauración en el repositorio Git del curso: `scripts/restore/restore_module3.sh`.
...
| RAM | Mínimo 8 GB (4 GB asignados a Docker) |
| Almacenamiento | Mínimo 20 GB libres (SSD recomendado) |
| Pantalla | 1920×1080 o superior |
...
| pgAdmin 4 | 7.x o superior | Interfaz gráfica para ejecutar consultas y visualizar planes |
| psql (CLI) | 16.x | Cliente de línea de comandos alternativo |
| Visual Studio Code | 1.85 o superior | Editor para escribir y guardar scripts SQL |
...
docker ps --filter "name=postgres_sales" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
```

Si el contenedor no está activo, inícialo:

```bash
# Iniciar el contenedor (desde el directorio del laboratorio 01-00-01)
docker compose up -d

# Esperar 5 segundos y verificar conectividad
sleep 5
docker exec -it postgres_sales psql -U postgres -d sales_db -c "SELECT version();"
```

Conectarse a la base de datos para el laboratorio:

```bash
# Opción A: Conectarse con psql desde el contenedor
docker exec -it postgres_sales psql -U postgres -d sales_db

# Opción B: Conectarse con psql local (si está instalado)
psql -h localhost -p 5432 -U postgres -d sales_db
```

> 🔒 **Nota de Seguridad:** Las credenciales `postgres/postgres` son exclusivas para este entorno de desarrollo local. **Nunca uses estas credenciales en un entorno de producción.** En producción, usa contraseñas complejas, roles con privilegios mínimos y conexiones cifradas con SSL/TLS.
...
**Objetivo:** Familiarizarse con las tablas base del dataset antes de crear vistas, identificando las relaciones y columnas disponibles para las consultas de negocio.

**Instrucciones:**

1. Conéctate a la base de datos `sales_db` y examina las tablas disponibles:
...
**Objetivo:** Crear tres vistas lógicas que encapsulen las consultas más frecuentes del dataset de ventas, estableciendo la primera capa de la arquitectura de vistas.
...
**Objetivo:** Comprender cuándo una vista es actualizable (updatable) en PostgreSQL y cuáles son sus limitaciones, usando la vista de regiones como ejemplo práctico.
...
2. Intenta realizar una actualización a través de la vista:
   ...
   -- Revertir el cambio para no afectar datos del laboratorio
   UPDATE analytics.vw_clientes_activos
   SET email = (
...
**Objetivo:** Establecer una línea base de rendimiento usando `EXPLAIN ANALYZE` para comparar el costo de consultas directas contra el uso de vistas lógicas, preparando el contexto para demostrar el valor de las vistas materializadas.
...
**Objetivo:** Implementar una vista materializada que precalcule el resumen mensual de ventas, persistiendo los resultados en disco para consultas instantáneas desde Power BI y otros clientes.

**Instrucciones:**

1. Crea la vista materializada principal del laboratorio:
   ...
   > ⏱️ **Nota:** La creación de la vista materializada ejecuta la consulta completa. Esto puede tardar varios segundos dependiendo del volumen de datos. Es normal.
...
**Objetivo:** Usar `EXPLAIN ANALYZE` para visualizar y documentar la diferencia de rendimiento entre la consulta directa y la vista materializada, comprendiendo por qué la materialización es tan efectiva.
...
**Objetivo:** Aprender a actualizar los datos de una vista materializada usando `REFRESH MATERIALIZED VIEW` y `REFRESH MATERIALIZED VIEW CONCURRENTLY`, comprendiendo las diferencias de bloqueo y cuándo usar cada opción.
...
**Objetivo:** Construir la arquitectura completa de capas de vistas (base → negocio → materializada) que servirá como fuente de datos optimizada para los dashboards de Power BI en el Módulo 6.
...
### Paso 9: Ejercicio de Reto — Estrategia de Refresh Automatizado

**Objetivo:** Aplicar el conocimiento adquirido para diseñar una estrategia de actualización de vistas materializadas usando funciones de PostgreSQL, preparando el terreno para el Módulo 4 de procedimientos almacenados.

> 🏆 **Nivel de Dificultad:** Fácil-Intermedio | **Este ejercicio se evalúa antes de avanzar al siguiente laboratorio**

**Instrucciones:**

1. Crea una función que refresque todas las vistas materializadas del esquema `analytics`:

   ```sql
   -- Función para refrescar todas las vistas materializadas del esquema analytics
   CREATE OR REPLACE FUNCTION analytics.fn_refresh_all_views()
   RETURNS TABLE (
       vista_nombre    TEXT,
       estado          TEXT,
       tiempo_ms       NUMERIC
   ...
   DECLARE
       ...
       v_vista     RECORD;
   BEGIN
       -- Iterar sobre todas las vistas materializadas del esquema analytics
       FOR v_vista IN
           SELECT matviewname
           FROM pg_matviews
           WHERE schemaname = 'analytics'
       ...
       LOOP
           ...
           -- Refrescar la vista materializada actual
           EXECUTE format(
           ...
           -- Retornar resultado de esta vista
           RETURN QUERY SELECT
               v_vista.matviewname::TEXT,
               'COMPLETADO'::TEXT,
               ROUND(
   ...
   SELECT * FROM analytics.fn_refresh_all_views();
   ```

2. Verifica el resultado de la función:

   ```sql
   -- Confirmar que todas las vistas fueron actualizadas
   SELECT
       matviewname,
       ispopulated
   FROM pg_matviews
...
1. Verifica la existencia y estado de todos los objetos del laboratorio:
...
**Causa:**
Las tablas base (`sales`, `customers`, `products`, `regions`) no existen en la base de datos `sales_db`, o el estudiante está conectado a una base de datos incorrecta. Esto ocurre cuando los laboratorios del Módulo 2 y 3 no fueron completados correctamente.

**Solución:**

```bash
# Verificar la base de datos actual
docker exec -it postgres_sales psql -U postgres -c "\l"
...
docker exec -i postgres_sales psql -U postgres -d sales_db \
...
**Causa:**
El usuario conectado no tiene permisos sobre el esquema `analytics`. En el entorno del laboratorio, esto puede ocurrir si se conectó con un usuario diferente a `postgres`.
...
> ⚠️ **Advertencia:** Ejecuta los comandos de limpieza **SOLO** si deseas eliminar todos los objetos creados en este laboratorio. Los laboratorios posteriores (04-00-02 y siguientes) dependen de las vistas y vistas materializadas creadas aquí. **No ejecutes la limpieza si planeas continuar con el siguiente laboratorio.**

```sql
-- ============================================================
-- LIMPIEZA COMPLETA DEL LABORATORIO 04-00-01
-- EJECUTAR SOLO SI SE DESEA REHACER EL LABORATORIO DESDE CERO
-- ============================================================
...
- **Lab 04-00-02:** Creación de procedimientos almacenados y funciones en PL/pgSQL — aprenderás a encapsular lógica de negocio compleja en funciones reutilizables, incluyendo la automatización del refresh de vistas materializadas con manejo de errores y logging
- **Lab 05-00-01:** Indexación y optimización de consultas — profundizarás en el uso de `EXPLAIN ANALYZE` y aprenderás a crear índices estratégicos para maximizar el rendimiento con 500K registros
- **Lab 06-00-01:** Integración con Power BI — conectarás Power BI Desktop directamente a las vistas materializadas creadas en este laboratorio para construir dashboards interactivos
