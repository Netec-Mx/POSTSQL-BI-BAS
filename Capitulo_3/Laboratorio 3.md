# Laboratorio 3: Uso de Agrupaciones Avanzadas
...
- Laboratorio 03-00-02 completado exitosamente (el dataset de ventas debe estar poblado y las consultas de subconsultas/CTEs del laboratorio anterior deben haber funcionado correctamente)
...
- Acceso a pgAdmin 4 (http://localhost:5050) con las credenciales configuradas
- Dataset de ventas creado en el laboratorio 02-00-01 con al menos 10,000 registros
...
| Almacenamiento | Mínimo 5 GB libres |
...
| PostgreSQL | 16.x (imagen Docker) | Motor de base de datos |
| pgAdmin 4 | 7.x o superior | Interfaz gráfica para ejecutar consultas |
| psql (CLI) | 16.x | Validación desde línea de comandos |
...
> ⚠️ **Nota de seguridad:** Las credenciales usadas en este laboratorio (`usuario: postgres`, `contraseña: postgres`) son exclusivamente para entornos de desarrollo local. **Nunca uses estas credenciales en un entorno de producción.**
...
**Objetivo:** Confirmar que el dataset de ventas contiene los datos necesarios para los ejercicios de agregación avanzada, y crear las estructuras de apoyo que se usarán durante el laboratorio.

**Instrucciones:**

1. Abre pgAdmin 4 en tu navegador (http://localhost:5050) e inicia sesión con las credenciales configuradas en el laboratorio 01-00-01.
...
**Objetivo:** Usar `GROUPING SETS` para generar en una sola consulta los totales de ventas por región, por categoría, por la combinación región+categoría, y el gran total general.
...
**Objetivo:** Aplicar `ROLLUP` para generar un reporte de ventas que muestre subtotales progresivos por año → trimestre → mes, siguiendo una jerarquía temporal natural.
...
**Objetivo:** Usar `CUBE` para generar una tabla de contingencia completa que muestre el total de ventas para todas las combinaciones posibles de las dimensiones región, categoría y año.
...
**Objetivo:** Aplicar la cláusula `FILTER` en funciones de agregación para calcular múltiples métricas condicionales en una sola pasada sobre los datos, evitando múltiples subconsultas o expresiones `CASE WHEN`.
...
**Objetivo:** Aplicar funciones estadísticas de PostgreSQL (`STDDEV`, `VARIANCE`, `CORR`, `PERCENTILE_CONT`) para derivar KPIs financieros que van más allá de las métricas básicas de suma y promedio.
...
Al finalizar el laboratorio, ejecuta los siguientes comandos para limpiar los objetos creados durante las prácticas y dejar el entorno listo para el siguiente laboratorio:

```sql
-- Eliminar la vista creada en el Paso 1 (se recreará si es necesaria en laboratorios posteriores)
...
```bash
# Verificar el estado del contenedor Docker (debe seguir en ejecución para laboratorios posteriores)
docker ps --filter "name=postgres" --format "table {{.Names}}\t{{.Status}}"
```

> ⚠️ **Advertencia:** **No detengas el contenedor Docker** al finalizar este laboratorio. El dataset de ventas y la configuración del servidor PostgreSQL son necesarios para los laboratorios 03-00-04 y siguientes. Solo ejecuta `docker compose down` si necesitas apagar tu equipo, y recuerda reiniciar los contenedores con `docker compose up -d` antes del próximo laboratorio.

> ⚠️ **Nota:** La columna `tipo_cliente` agregada en el Paso 1 se conserva en la tabla `ventas` ya que será útil en laboratorios posteriores. Si deseas eliminarla, ejecuta: `ALTER TABLE ventas DROP COLUMN IF EXISTS tipo_cliente;`
...
- Completa el **ejercicio de reto** a continuación antes de avanzar al laboratorio 03-00-04
- En el **Laboratorio 03-00-04** (TimescaleDB) aplicarás estas técnicas de agregación en el contexto de series de tiempo con extensiones especializadas de PostgreSQL
...
> 📝 **Nota para el instructor:** La solución de este reto está disponible en el repositorio Git del curso bajo `solutions/03-00-03-challenge-solution.sql`. No compartir con los estudiantes hasta que completen el ejercicio de forma independiente.
