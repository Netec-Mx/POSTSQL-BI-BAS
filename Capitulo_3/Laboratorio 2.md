# Laboratorio 2: Aplicación de Funciones de Ventana
...
| **Bloom Level** | Aplicar |
| **ID de Laboratorio** | 03-00-02 |
| **Módulo** | 3 – SQL Analítico Avanzado |
| **Dependencia** | Laboratorio 03-00-01 completado |
...
- Laboratorio 03-00-01 completado exitosamente (dataset de ventas con datos temporales de 24 meses disponible)
...
- Acceso a pgAdmin 4 o psql CLI con conexión activa a la base de datos `sales_db`
- Base de datos `sales_db` con el esquema y datos del laboratorio 03-00-01 cargados
...
| Almacenamiento | Mínimo 2 GB libres para datos de laboratorio |
| Procesador | Intel Core i5 / AMD Ryzen 5 o superior |
| Pantalla | 1920x1080 recomendado para pgAdmin 4 |
...
| pgAdmin 4 | 7.x o superior | Interfaz gráfica para ejecutar consultas |
| psql CLI | 16.x | Cliente de línea de comandos alternativo |
| Visual Studio Code | 1.85 o superior | Editor para scripts SQL (opcional) |
...
docker ps --filter "name=postgres_sales"

# Si el contenedor no está activo, iniciarlo
docker start postgres_sales

# Verificar conectividad a la base de datos
docker exec -it postgres_sales psql -U postgres -d sales_db -c "SELECT COUNT(*) FROM ventas;"
```

Si el comando anterior devuelve un conteo de registros (mínimo esperado: varios miles de filas con 24 meses de datos), el entorno está listo. Si la tabla `ventas` no existe o está vacía, ejecuta el script de restauración del laboratorio 03-00-01 antes de continuar.

```bash
# Verificar que existen datos temporales de al menos 24 meses
docker exec -it postgres_sales psql -U postgres -d sales_db -c "
SELECT 
...
**Objetivo:** Confirmar que las tablas necesarias existen con la estructura correcta y que los datos temporales abarcan al menos 24 meses. Si los datos son insuficientes, se generarán datos adicionales.

**Instrucciones:**

1. Conéctate a la base de datos `sales_db` desde psql:
...
**Objetivo:** Comprender la sintaxis base de las funciones de ventana y observar la diferencia entre agregación con `GROUP BY` y agregación con `OVER`, donde el detalle de cada fila se preserva.
...
**Objetivo:** Aplicar las tres funciones de ranking principales para clasificar productos por ventas dentro de cada categoría, entendiendo las diferencias de comportamiento cuando existen empates.
...
1. Primero, crea una vista materializada con ventas agregadas por producto y categoría para facilitar los ejercicios de ranking:
   ...
   ```sql
   -- Vista de ventas por producto (usada en múltiples ejercicios de este laboratorio)
...
**Objetivo:** Usar `LAG()` para acceder al valor de la fila anterior y calcular la variación porcentual de ventas mes a mes por vendedor, identificando meses de crecimiento y decrecimiento.
...
**Objetivo:** Implementar promedios móviles de 3 y 6 meses usando la sintaxis `ROWS BETWEEN` para suavizar series de tiempo y eliminar ruido en los datos de ventas.
...
**Objetivo:** Aplicar funciones de distribución para segmentar clientes en cuartiles con `NTILE(4)`, calcular el percentil relativo de cada cliente con `PERCENT_RANK()` y la distribución acumulada con `CUME_DIST()`.
...
**Objetivo:** Usar la cláusula `WINDOW` al final de la consulta para definir una ventana una sola vez y referenciarla en múltiples funciones, mejorando la legibilidad y evitando redundancia en consultas con muchas funciones de ventana.
...
4. Guarda el reporte como una vista para uso posterior en laboratorios de Power BI:

   ```sql
   -- Crear vista del reporte de rendimiento de vendedores
   -- Esta vista será usada en el laboratorio 06-00-01 de Power BI
...
**Objetivo:** Consolidar todas las técnicas aprendidas en una consulta analítica compleja que simula un reporte ejecutivo de ventas, combinando ranking, LAG/LEAD, promedios móviles y segmentación.

**Instrucciones:**

1. Ejecuta la consulta integradora que combina todas las técnicas del laboratorio:
...
1. Verificar que todas las vistas del laboratorio existen:

   ```sql
   -- Listar todas las vistas creadas en este laboratorio
   SELECT 
       table_name AS vista,
       table_type
...
Al finalizar el laboratorio, las vistas creadas son parte del dataset que se reutilizará en laboratorios posteriores. **No elimines las vistas** a menos que necesites reiniciar el laboratorio desde cero.

```sql
-- OPCIONAL: Eliminar vistas solo si necesitas reiniciar el laboratorio
-- ADVERTENCIA: Estas vistas son necesarias para laboratorios posteriores
...
```bash
# Si necesitas reiniciar completamente el laboratorio, restaura desde el backup del Lab 03-00-01
docker exec -it postgres_sales psql -U postgres -d sales_db -f /scripts/lab_03_00_01_backup.sql
```

> ⚠️ **Advertencia:** Las vistas `v_reporte_rendimiento_vendedores` y `v_reporte_ejecutivo_ventas` son utilizadas en el **Laboratorio 06-00-01** de Power BI. Eliminarlas impedirá conectar los dashboards de Power BI correctamente. Si debes eliminarlas, asegúrate de recrearlas antes de continuar con el Laboratorio 06-00-01.

> ⚠️ **Nota de Seguridad:** Este laboratorio utiliza credenciales simplificadas (`usuario: postgres`, `contraseña: postgres`) exclusivamente para entornos de desarrollo local. **Nunca uses estas credenciales en entornos de producción.** En producción, usa contraseñas robustas, roles con privilegios mínimos y conexiones SSL.
...
- Creaste 5 vistas analíticas que serán reutilizadas en laboratorios posteriores, incluyendo el laboratorio de Power BI
...
### Reto Final (Evaluación Formativa)
...
- **Laboratorio 03-00-03:** Aplicación de funciones de agregación avanzadas y análisis de KPIs financieros usando `FILTER`, `GROUPING SETS`, `ROLLUP` y `CUBE`
- **Laboratorio 03-00-04:** Introducción a análisis de series de tiempo con TimescaleDB y funciones temporales avanzadas de PostgreSQL
- **Laboratorio 06-00-01:** Las vistas creadas en este laboratorio (`v_reporte_rendimiento_vendedores`, `v_reporte_ejecutivo_ventas`) serán conectadas a Power BI Desktop para construir dashboards interactivos
