# Laboratorio 4: Uso de Funciones de Tiempo 
...
| **Bloom Level** | Aplicar |
| **Laboratorio Anterior** | 03-00-03 (requerido) |
| **Laboratorio Siguiente** | 04-00-01 |

## Descripción General

En este laboratorio aplicarás técnicas avanzadas de análisis temporal en PostgreSQL utilizando el dataset de ventas enriquecido construido en laboratorios anteriores. Dominarás funciones clave como `DATE_TRUNC()`, `EXTRACT()`, `INTERVAL` y `generate_series()` para realizar agrupaciones temporales, detectar gaps en datos, construir comparaciones Year-over-Year (YoY) y Month-over-Month (MoM), e implementar un análisis de cohortes de clientes. Finalizarás explorando TimescaleDB, una extensión de PostgreSQL diseñada específicamente para series de tiempo, aprendiendo a crear hypertables y ejecutar consultas optimizadas con `time_bucket()`.
...
- Laboratorios 03-00-01, 03-00-02 y 03-00-03 completados exitosamente
- Dataset de ventas (`sales`, `customers`, `products`, `order_items`) disponible en la base de datos `sales_db`
...
- Permisos de superusuario en la base de datos `sales_db` (usuario `postgres`)
- Conexión a internet para descargar la imagen Docker de TimescaleDB (o imagen pre-descargada disponible)

> ⚠️ **Advertencia:** Si no completaste los laboratorios 03-00-01 al 03-00-03, ejecuta el script de setup `lab_03_00_04_setup.sql` antes de continuar. Este script restaura el estado esperado de la base de datos.
...
| Red | Conexión a internet para imagen TimescaleDB |

### Requisitos de Software

| Software | Versión | Propósito |
|----------|---------|-----------|
| Docker Desktop | 4.25.0 o superior | Ejecutar contenedores PostgreSQL y TimescaleDB |
| PostgreSQL | 16.x (imagen Docker) | Motor de base de datos principal |
| TimescaleDB | imagen `timescale/timescaledb-ha:pg16` | Extensión para series de tiempo |
| pgAdmin 4 | 7.x o superior | Interfaz gráfica para ejecutar consultas |
| psql (CLI) | 16.x | Cliente de línea de comandos alternativo |
| Docker Compose | 2.x (incluido en Docker Desktop) | Orquestación de contenedores |
...
docker exec -it postgres_db psql -U postgres -d sales_db -c "\dt"
```
...
> 💡 **Nota sobre TimescaleDB:** La sección final de este laboratorio (Paso 7) requiere un contenedor Docker diferente al PostgreSQL estándar. Se utilizará la imagen `timescale/timescaledb-ha:pg16`. Puedes tener ambos contenedores ejecutándose simultáneamente en puertos diferentes. El contenedor TimescaleDB usará el puerto `5433`.
...
**Objetivo:** Verificar que el dataset de ventas contiene columnas de timestamp con suficiente variedad temporal para los análisis posteriores, y enriquecer los datos si es necesario.

**Instrucciones:**

1. Abre pgAdmin 4 en tu navegador (http://localhost:5050) y conéctate al servidor PostgreSQL. Selecciona la base de datos `sales_db` y abre el Query Tool.
...
**Objetivo:** Dominar el uso de `DATE_TRUNC()` para agregar ventas a diferentes granularidades temporales: día, semana, mes, trimestre y año.
...
**Objetivo:** Utilizar `EXTRACT()` para obtener componentes específicos de fechas y construir análisis de estacionalidad multidimensional.
...
**Objetivo:** Construir métricas de crecimiento temporal comparando períodos equivalentes usando CTEs anidadas y la función de ventana `LAG()`.
...
**Objetivo:** Usar `generate_series()` para crear un calendario completo de fechas y detectar mediante `LEFT JOIN` los días sin actividad de ventas.
...
**Objetivo:** Implementar un análisis de cohortes que agrupe clientes por su mes de primera compra y calcule la retención en meses subsiguientes.
...
**Objetivo:** Configurar un contenedor Docker con TimescaleDB, instalar la extensión, crear una hypertable y ejecutar consultas básicas con `time_bucket()` para comprender las ventajas de TimescaleDB en análisis de series de tiempo.
...
1. Verifica los objetos creados en `sales_db`:

```sql
-- Ejecutar en sales_db (puerto 5432)
SELECT 
...
**Causa:**
El laboratorio fue ejecutado previamente o la columna fue creada manualmente. La cláusula `IF NOT EXISTS` en `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` debería prevenir esto, pero versiones antiguas de PostgreSQL pueden no soportarla.
...
Al finalizar el laboratorio, puedes optar por mantener los objetos creados (son necesarios para laboratorios posteriores) o limpiar el entorno de TimescaleDB si no lo necesitas inmediatamente.

```sql
-- OPCIONAL: Eliminar vistas creadas en este laboratorio (NO recomendado si continuarás al Lab 04)
...
> ⚠️ **Advertencia:** NO elimines las vistas `v_metricas_temporales` y `v_analisis_cohortes` ni la columna `sale_timestamp` si planeas continuar con el laboratorio 04-00-01. Estos objetos son referenciados en los scripts de setup del siguiente laboratorio. El contenedor `timescaledb_lab` puede detenerse sin problema y reiniciarse cuando sea necesario.

> 🔒 **Nota de Seguridad:** Los contenedores de este laboratorio usan la contraseña `postgres` para facilitar el aprendizaje. Esta configuración es **exclusivamente para entornos de desarrollo local**. En ambientes de producción, utiliza contraseñas robustas, certificados SSL/TLS, y restricciones de red apropiadas.
...
- Continúa con el **Laboratorio 04-00-01** donde aplicarás las vistas y métricas creadas aquí para construir funciones y procedimientos almacenados en PL/pgSQL
- Practica el reto de cohortes de la siguiente sección para consolidar tu comprensión antes de avanzar
- Explora la documentación oficial de TimescaleDB para conocer características avanzadas como políticas de compresión automática y vistas continuas (continuous aggregates)
...
> **Instrucción:** Completa este reto antes de avanzar al laboratorio 04-00-01. No se provee solución; el instructor evaluará tu implementación.
