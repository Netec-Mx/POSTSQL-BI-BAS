# Laboratorio 1: Instalación de imagen PostgreSQL en Docker
...
> ⚠️ **Nota de Seguridad:** Este laboratorio utiliza credenciales simplificadas (`postgres`/`postgres`) para facilitar el aprendizaje. Estas configuraciones son **exclusivamente para entornos de desarrollo local** y **jamás deben usarse en producción**. En producción, siempre usa contraseñas complejas, roles con privilegios mínimos y conexiones cifradas con TLS.
...
**Objetivo:** Confirmar que Docker Desktop está correctamente instalado, en ejecución y que Docker Compose está disponible antes de continuar con el laboratorio.
...
**Objetivo:** Organizar los archivos del laboratorio en una estructura de directorios clara que se usará durante todo el curso.
...
**Objetivo:** Definir la configuración completa del entorno multi-contenedor con PostgreSQL 16 y pgAdmin 4, incluyendo volúmenes persistentes y red interna de Docker.
...
**Objetivo:** Iniciar los contenedores de PostgreSQL y pgAdmin, verificar que ambos servicios están en ejecución y que el healthcheck de PostgreSQL pasa correctamente.
...
> ⏱️ **Tiempo de espera:** pgAdmin puede tardar entre 30 y 60 segundos en estar completamente disponible después de que PostgreSQL pase su healthcheck. Si el contenedor de pgAdmin muestra `starting` en lugar de `Up`, espera unos segundos y vuelve a ejecutar `docker compose ps`.

---

### Paso 5: Conectarse a PostgreSQL con psql desde el contenedor

**Objetivo:** Realizar la primera conexión a PostgreSQL usando el cliente `psql` desde dentro del contenedor Docker y explorar los meta-comandos fundamentales.
...
**Objetivo:** Crear una tabla simple y insertar datos para verificar que los volúmenes Docker están funcionando correctamente y que los datos persisten entre reinicios del contenedor.
...
**Objetivo:** Acceder a la interfaz web de pgAdmin 4 y configurar una conexión al servidor PostgreSQL para administrar la base de datos de forma gráfica.
...
**Objetivo:** Usar consultas SQL del catálogo del sistema para identificar los componentes principales de la arquitectura de PostgreSQL: procesos, configuración de memoria y parámetros del motor.
...
> ⚠️ **Advertencia Importante:** Los siguientes comandos detendrán los contenedores. **NO ejecutes los comandos que eliminan volúmenes** si quieres conservar los datos para los laboratorios posteriores. El entorno configurado en este laboratorio se reutiliza en todos los módulos del curso.
...
- Avanza al **Laboratorio 02-00-01** donde crearás el dataset de ventas que se usará durante todo el curso, aplicando los conceptos de diseño de esquemas relacionales.
- Explora la **documentación oficial de PostgreSQL 16** en [postgresql.org/docs/16](https://www.postgresql.org/docs/16/) para profundizar en los parámetros de configuración que exploraste en el Paso 8.
- Revisa la **Lección 1.2** sobre la arquitectura interna de PostgreSQL para entender en mayor profundidad los componentes que identificaste: postmaster, shared buffers, WAL y query planner.

---

## 🏆 Ejercicio de Reto (Sin Solución Provista)

> **Nivel de dificultad:** ⭐⭐ Básico-Intermedio
> 
> **Instrucciones para el instructor:** Este ejercicio debe completarse antes de avanzar al Laboratorio 02-00-01 en un contexto de curso presencial. Evalúa la comprensión del estudiante sobre Docker Compose, variables de entorno y psql.

**Desafío:** Modifica el entorno Docker para agregar una segunda base de datos de práctica llamada `analytics_db` al mismo servidor PostgreSQL, y crea un usuario dedicado `analista` con contraseña `analista123` que tenga acceso completo únicamente a `analytics_db` (no a `curso_db`).

**Criterios de evaluación:**
1. El usuario `analista` puede conectarse a `analytics_db` con `psql`
2. El usuario `analista` NO puede conectarse a `curso_db`
3. La segunda base de datos persiste después de reiniciar el contenedor
4. Documenta en un archivo `notas.md` los comandos SQL usados y explica por qué elegiste ese enfoque

**Pistas:**
- Investiga las variables de entorno `POSTGRES_MULTIPLE_DATABASES` (no existe por defecto, deberás encontrar otra forma)
- Consulta la documentación de PostgreSQL sobre `CREATE USER`, `CREATE DATABASE` y `GRANT`
- El comando `\c` de psql y la opción `-U` serán útiles para probar los permisos
